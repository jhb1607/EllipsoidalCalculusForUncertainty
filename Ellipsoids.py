    ...
    def plot(self, ax=None, label=None, color=None, alpha=0.1):
        ...
        # For 3D ellipsoids
            ...
            if not hasattr(ax, 'plot_surface'):
                print("Warning: Trying to plot 3D ellipsoid on 2D axes. Creating new figure with 3D axes.")
                fig = plt.figure()
                ax = fig.add_subplot(111, projection='3d')
            
            u = np.linspace(0, 2*np.pi, 20)  # Reduziert auf 20 für bessere Performance
